...
- **Chunked consolidation for very long chats**: Long roleplays with large memory sets no longer truncate on Consolidate. The extension now auto-detects when a memory set would overflow a single LLM call and switches transparently to a map-reduce pipeline: memories are split into char-budgeted chunks, each consolidated independently, then a final reduce pass deduplicates across chunks. Small memory sets continue to use the existing single-call path (no latency change). Adds an Advanced setting *Consolidation chunk size (chars)* (default 24000, minimum 4000) for tuning against providers with different output limits. The Consolidate button doubles as Cancel while chunked mode is running, stopping at the next chunk boundary. Follow-up to [#13](https://github.com/bal-spec/sillytavern-character-memory/issues/13) — the 2.1.10 fix raised `max_tokens` but chunked consolidation is the architectural fix for users with very long roleplays (4000+ messages).
...
- **Set as last-extracted point**: New per-message button (map-pin icon) on character messages that moves the extraction pointer without running extraction — the inverse of "Extract memories up to here". Rewind it after deleting a bad memory chunk so those messages get picked up again, or fast-forward it after manually reconciling the Data Bank, without discarding tracking for the rest of the chat. Shows a confirm dialog with the current and target positions, and notes in group chats that all members share one pointer. Fixes [#20](https://github.com/bal-spec/sillytavern-character-memory/issues/20).

### Security

- **Fix stored HTML injection in the dashboard health summary**: `updateDashboardDiagSummary()` was the only one of six renderers of the health-check data that inserted check labels and details without `escapeHtml()`. A custom Data Bank filename containing HTML or script tags — free text, embedded verbatim into a check's detail — would execute on every chat switch and every 60-second health poll. Also fixes an adjacent always-true filter (`c.status !== 'pass'` against objects that only ever set `.level`) that made every check render there regardless of actual health.

### Bug Fixes

- **Fix data loss when a Data Bank write fails**: `writeMemoriesForCharacter()` deleted the existing memory file *before* uploading its replacement, so an upload that failed after the delete succeeded lost the memories entirely, with no rollback or backup. The upload now happens first and the old file is removed only once the new one is confirmed. The delete is additionally wrapped so a failure there can't skip linking the successfully-uploaded file into settings.
- **Fix silently clobbered edits from concurrent writes**: The Memory Manager, Consolidation, Reformat, and the Troubleshooter's file editor each read a memory file once, stay open while you edit, and previously wrote the result straight back with no re-check. A background auto-extraction landing in the meantime was silently overwritten. Each now re-reads the file immediately before saving and asks before overwriting if it changed.
- **Fix double-triggering Extract, Consolidate, and Reformat**: The reentrancy guard on each was checked at entry but not claimed until deep inside, after several awaited dialogs — so a fast double-click could pass the check twice and run two concurrent operations, producing duplicate writes and an unpredictable extraction pointer. Each now claims its lock before any await, released on every exit path including thrown errors.
- **Fix extraction writing to the wrong chat after a mid-run chat switch**: Extraction re-validated the active chat only once, right after the LLM call, but performed several more awaited operations before committing a chunk's bookkeeping. Switching chats during those could write this extraction's pointer and hidden-message state into whichever chat became active. Context is now re-checked before every metadata write.
- **Fix extraction silently stopping after messages are deleted**: Nothing adjusted `lastExtractedIndex` when messages were removed, so a chat that shrank past the stored pointer reported "no new messages" forever — no error, no warning. The pointer now self-heals to the current chat length on next access.
- **Fix batch extraction colliding between same-named characters**: Batch progress was keyed by character display name, which SillyTavern doesn't enforce as unique. Two cards sharing a name (duplicate imports, common persona names) collided — one character's progress became the other's "already processed" boundary, permanently skipping messages. Now keyed by avatar, with a one-time migration of existing records. Records that can't be attributed to exactly one character are left untouched rather than guessed at.
- **Fix theme labels containing `>` corrupting their own memory block**: Chat and theme labels are free text and sometimes LLM-generated, but attribute escaping only handled `&` and `"`. A literal `>` ended the `<memory chat="…">` opening tag early, corrupting the block on the next parse.
- **Surface unexpected provider responses instead of hiding them**: A 200 response with no `choices`/`message` (or no `content` array, for Anthropic) returned an empty string, which read upstream as "no new memories found" — indistinguishable from a legitimate empty result, silently masking a real API problem. These now raise an error. A present-but-empty completion is still treated as legitimate.
- **Stop saving refusals and malformed output as memories**: When an LLM response contained no bulleted lines, both extraction and consolidation fell back to storing the entire raw response as a single memory — persisting things like "I can't help with that" as if they were extracted facts. Un-bulleted entries are now skipped and logged.
- **Make "Stop extraction" cancel the in-flight request**: The abort signal was only checked between chunks, never passed to the request itself, so stopping had no effect until the current call finished on its own. It now threads through to the underlying `fetch` for the Dedicated API provider path.
- **Make chunked consolidation actually retry failed chunks**: The retry orchestrator only activated on a thrown error, but the LLM call caught everything internally and returned null — so a transient network blip silently dropped that chunk's memories instead of retrying. Failures now propagate so retries engage, with a toast once they're exhausted.
- **Fix the consolidation block picker always returning an empty selection**: "Change selection…" read its checkboxes after the popup had already torn down its DOM, so it always came back empty and the caller concluded "nothing to do" — the same root cause as [#14](https://github.com/bal-spec/sillytavern-character-memory/issues/14), missed in this dialog.
- **Fix group chats missing per-message buttons and a broken conversion flow**: Pin, Extract-here, and View-injected never appeared on any message in a group chat, and the Convert tool failed with "No character selected", because both paths tested `characterId`, which SillyTavern leaves unset in group chats outside a live generation turn. Health checks and the diagnostic report also only ever described the first member; they now cover every member.
- **Fix the injection drawer reopening after you closed it**: Opening the log drawer force-closed the injection drawer through raw DOM manipulation without clearing its persisted state, so it incorrectly auto-reopened on the next page load.
- **Fix drag-to-resize closing the drawer on touch devices**: The resize handle's touch events also reached the drawer's swipe-to-close listener, so dragging to resize could satisfy the swipe distance and close the drawer being resized.
- **Fix duplicate Setup Wizard popups**: The wizard is reachable from three independent triggers plus two deferred re-opens and had no reentrancy guard, so a fast double-trigger could open two at once.
